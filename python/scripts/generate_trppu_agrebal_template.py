"""Génère le template Excel pour l'upload massif de trppu_agrebal.

Usage :
    python scripts/generate_trppu_agrebal_template.py
    python scripts/generate_trppu_agrebal_template.py --output path/to/file.xlsx
"""
...
HEADERS = ["id_agrebal", "co_regate", "lb_agrebal", "est_actif"]
EXAMPLE_ROWS = [
    (50001, "012345", "Amas Paris Nord-Centre", 1),
    (50002, "012345", "Amas Paris Sud-Banlieue", 1),
    (60001, "067890", "Amas Lyon Bellecour-Part-Dieu", 0),
]
COLUMN_WIDTHS = {"id_agrebal": 14, "co_regate": 12, "lb_agrebal": 40, "est_actif": 12}
...
def build_workbook() -> Workbook:
    ...
    notice["A1"] = "Template d'import — table trppu_agrebal"
    notice["A1"].font = Font(bold=True, size=14)
    instructions = [
        "",
        "Définition : un amas est un regroupement géographique de PDI desservis de façon continue.",
        "",
        "Colonnes :",
        " - id_agrebal : identifiant amas (entier > 0, obligatoire, clé primaire)",
        " - co_regate  : code Regate du site parent (6 caractères, obligatoire, doit exister dans trppu_site)",
        " - lb_agrebal : libellé (max 120 caractères, optionnel)",
        " - est_actif  : 1 = actif (défaut), 0 = inactif",
        "",
        "Comportement de l'import :",
        " - Upsert sur id_agrebal (INSERT ... ON DUPLICATE KEY UPDATE).",
        ...
        "",
        "Endpoint :",
        " - POST /trppu-api/agrebals/upload-excel  (multipart/form-data, champ 'file')",
    ]
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="trppu_agrebal_template.xlsx",
        help="Chemin du fichier Excel à générer (défaut : ./trppu_agrebal_template.xlsx)",
    )
