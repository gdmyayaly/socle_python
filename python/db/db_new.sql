...
-- 25 tables — schéma uniquement (sans données)
--
-- Organisation du fichier :
--   1. Référentiel / socle        : produit, site, referentiel, version_cle
--   2. Coefficients PIC           : pic_version, pic_coefficients(_ko)
--   3. Scénarios                  : scenario et ses tables filles
...
-- Ancienne structure (coefficients en colonnes dense/faible1/faible2), conservée pour reprise.
...
-- Table de chargement brut (toutes colonnes en text) + colonnes de rejet.
...
DROP TABLE IF EXISTS `trppu_suivi_batch`;
CREATE TABLE `trppu_suivi_batch` (
  ...
  KEY `idx_batch` (`lb_batch`,`dt_debut`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
