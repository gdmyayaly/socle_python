...
| `PicCoefficientService` | **Aucun (simulé)** | Coefficients PIC par (produit, jour, densité). Grille par défaut codée en dur + overrides par scénario **persistés en localStorage** avec `delay()` pour simuler le réseau. Tickets DSR-660 / DSR-661 — à brancher sur le vrai back (service YS04). |
...
12. **Mocks à brancher** : `PicCoefficientService` (localStorage + grille en dur) et `ParamService` (assets JSON) attendent leur backend.
