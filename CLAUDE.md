...
Monorepo with seven independent sub-projects (part of the La Poste / DSR "TRPPU" ecosystem — TRPPU = average traffic per product for a postal site, computed via parameterizable scenarios). Code comments, docs, and Jira tickets are in French.

- **`python/`** — the main, active project: FastAPI backend ("trppu API", module YS04) backed by MySQL and Databricks SQL Warehouse.
- **`yb04/`** — module YB04: a console batch that **initialises `trppu_cles_repartition`** — S3 access (`app/services/s3.py`, the only `boto3` use in the repo) and `charger-cles-repartition`, which streams a CSV from S3 into the table (22.4 M rows, purge then batched commits). Commands: `db-info`, `db-check`, `s3-check`, `charger-cles-repartition`. Has its own `yb04/README.md`. *(Was `yb06/` until 23/09/2026 — renamed because DSR-715/717 define YB06 as something else.)*
